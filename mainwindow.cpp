    ...
    ui(new Ui::MainWindow)
{
    ...
    ThreadManager& manager = ThreadManager::getInstance();  // 获取单例实例
    const int numberOfThreads = 3;  // 定义线程数量,设定为const防止numberOfThreads被意外修改

    // 创建多个线程和工作对象并加入线程管理器
    for (int i = 0; i < numberOfThreads; ++i) {
        manager.addWorker(i);  // 添加带有ID的工作对象
    }

    // 开始执行第一个线程
    manager.startNextThread();
...
void MainWindow::on_pushButton_clicked()
{
    ...
    if(ret <= 0){
        ...
        qDebug()<<QString().sprintf("Device connected succeed!");
    }
    //打开设备
    ...
    if(!state){
        ...
        qDebug()<<QString().sprintf("Open device succeed!");
    }
#if GET_FIRMWARE_INFO
    ...
    if(!state){
        ...
        qDebug()<<QString().sprintf("Firmware SerialNumber:%s",DevInfo.SerialNumber);
    }
#endif
